...
import { Search, Star, Eye, EyeOff, Loader2 } from 'lucide-react';
...
export default function HomepageProductsPage({ products, onToggleProductFeatured }: Props) {
  ...
  const featuredCount = products.filter(p => p.featuredOnHome).length;

  const toggle = async (id: string, current: boolean) => {
    setLoading(id);
    try { await onToggleProductFeatured(id, !current); }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div>
        <div style={{ fontSize: 24, fontWeight: 700, color: '#1a1a1a' }}>Home Page — Products</div>
        <div style={{ fontSize: 13, color: '#888', marginTop: 4 }}>
          Choose which products are featured on the public home page.
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {(['all', 'featured'] as const).map(f => (
            <button key={f} onClick={() => { setFilter(f); setPage(1); }}
              ...
              {f === 'all' ? `All (${products.length})` : `Featured (${featuredCount})`}
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={{ backgroundColor: '#fff', border: '1px solid #e8e8e8', borderRadius: 12, overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '2.5fr 1fr 1fr 120px', padding: '10px 16px', borderBottom: '1px solid #e8e8e8', backgroundColor: '#fafafa' }}>
          {['PRODUCT', 'CATEGORY', 'PRICE', 'FEATURED'].map(h => (
            <div key={h} style={{ fontSize: 11, fontWeight: 600, color: '#888', letterSpacing: '0.05em', textTransform: 'uppercase' }}>{h}</div>
          ))}
        </div>
        {paginated.length === 0 ? (
          <div style={{ padding: '48px 0', textAlign: 'center', color: '#888', fontSize: 13 }}>No products found.</div>
        ) : paginated.map(p => (
          <div key={p.id} style={{ display: 'grid', gridTemplateColumns: '2.5fr 1fr 1fr 120px', padding: '12px 16px', borderBottom: '1px solid #f0f0f0', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ width: 44, height: 44, borderRadius: 8, overflow: 'hidden', border: '1px solid #e8e8e8', flexShrink: 0, backgroundColor: '#f5f5f5' }}>
                {p.imageUrl ? <img src={resolveImg(p.imageUrl)} alt={p.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, fontWeight: 700, color: '#e84b10', backgroundColor: '#fff3ee' }}>{p.name.charAt(0)}</div>}
              </div>
              <div>
                <div style={{ fontSize: 13, fontWeight: 600, color: '#1a1a1a' }}>{p.name}</div>
                <div style={{ fontSize: 11, color: '#888', marginTop: 1 }}>{p.subtitle || p.voltage || ''}</div>
              </div>
            </div>
            <div><span style={{ fontSize: 11, padding: '3px 8px', borderRadius: 4, backgroundColor: '#fff3ee', color: '#e84b10', fontWeight: 600 }}>{p.category}</span></div>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#1a1a1a' }}>${p.price?.toFixed(2)}</div>
            <div>
      ...
      {totalPages > 1 && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            <button onClick={() => setPage(p => Math.max(1,p-1))} disabled={page===1} style={{ width:28,height:28,border:'1px solid #e8e8e8',borderRadius:6,background:'#fff',cursor:'pointer',fontSize:13,display:'flex',alignItems:'center',justifyContent:'center' }}>‹</button>
            ...
            <button onClick={() => setPage(p => Math.min(totalPages,p+1))} disabled={page===totalPages} style={{ width:28,height:28,border:'1px solid #e8e8e8',borderRadius:6,background:'#fff',cursor:'pointer',fontSize:13,display:'flex',alignItems:'center',justifyContent:'center' }}>›</button>
          </div>
